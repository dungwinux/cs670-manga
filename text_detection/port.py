...
class TextDetection:
    def __init__(self, cwd):
        ...
        self.mts_level = 2
    def execute(self, input_relpath):
        img_path = self.input_base / input_relpath
        if not img_path.exists():
            print(f"Cannot find {img_path}. ({input_base=}, {input_relpath=})")
            return
        ...
        result = proc.text_detection(img_name, mts_level=self.mts_level, cv2_model=self.cv2_model)
        output = {
